...
namespace Comperio\DSQ\Compiler;


use Comperio\DSQ\Compiler\Map\LibraryAreaMap;
use Comperio\DSQ\Compiler\Map\LoanableMap;
use Comperio\DSQ\Compiler\Map\StandardNumberMap;
use Comperio\DSQ\Compiler\Map\SubjTypeMap;
...
/**
 * Class DNGCompiler: A compiler loaded with the configuration of DNG
 *
 * @package Comperio\DSQ\Compiler
 */
class DNGCompiler extends CompilerChain
{
    public function __construct()
    {
        $m = new MapBuilder;
        $luceneCompiler = new LuceneCompiler;
        $luceneCompiler
            ...
            ->map('class', $m->regexps(array(
                '/^"?\d+(\.\d*)?\*?"?$/'    => $luceneCompiler->getMap('dewey'),
                '/.*/'                      => $luceneCompiler->getMap('classtxt')
            )))
...
TPL
        ...
        parent::__construct($luceneCompiler, new LuceneQueryCompiler());
    }
}